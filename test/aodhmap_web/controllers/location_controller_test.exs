defmodule AodhmapWeb.LocationControllerTest do
  use AodhmapWeb.ConnCase

  alias Aodhmap.Engrand

  @create_attrs %{idName: "some idName", latitude: 120.5, locationName: "some locationName", longitude: 120.5}
  ...
  def fixture(:location) do
    {:ok, location} = Engrand.create_location(@create_attrs)
    location
  end
  ...
  describe "update location" do
    ...
    test "redirects when data is valid", %{conn: conn, location: location} do
      ...
      assert html_response(conn, 200) =~ "some updated idName"
    end
